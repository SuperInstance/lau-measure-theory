//! Agent probability spaces: measure-theoretic foundations for agent models.
//!
//! An agent probability space combines a measurable space with a probability measure
//! and associates it with an agent's belief state.
...
/// An agent probability space (Ω, 𝒜, P) with an associated agent identifier.
#[derive(Debug, Clone)]
pub struct AgentProbabilitySpace {
    agent_id: String,
    measure: Measure,
}

impl AgentProbabilitySpace {
    /// Create a new agent probability space.
    pub fn new(agent_id: &str, measure: Measure) -> Self {
        assert!(measure.is_probability(), "Must be a probability measure");
        AgentProbabilitySpace {
...
/// Compute the total variation distance between two agent probability spaces.
///
/// δ(P₁, P₂) = sup_A |P₁(A) - P₂(A)|
pub fn total_variation_distance(space1: &AgentProbabilitySpace, space2: &AgentProbabilitySpace) -> f64 {
...
pub fn kl_divergence(space1: &AgentProbabilitySpace, space2: &AgentProbabilitySpace) -> f64 {
    let mut kl = 0.0;
...
mod tests {
    ...
    fn test_total_variation() {
        ...
        let s1 = AgentProbabilitySpace::new("a", mu1);
        let s2 = AgentProbabilitySpace::new("b", mu2);
